...
def associate(detections, trackers, iou_threshold, velocities, previous_obs, vdc_weight):    
    ...
    # IoU between detections and trackers
    iou_matrix = iou_batch(detections, trackers)

    # Velocity direction consistency (optional)
    # If you are not using velocity or it's not relevant, you can remove this part
    ...
    # Filter out matched with low IoU
    matches = [m.reshape(1, 2) for m in matched_indices if iou_matrix[m[0], m[1]] >= iou_threshold]
    matches = np.concatenate(matches, axis=0) if matches else np.empty((0, 2), dtype=int)

    return matches, np.array(unmatched_detections), np.array(unmatched_trackers)

def associate_pigds(detections, trackers, det_cates, iou_threshold, 
